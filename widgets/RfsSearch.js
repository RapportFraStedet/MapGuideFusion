...
Fusion.Widget.RfsSearch = OpenLayers.Class(Fusion.Widget, {
		...
		initializeWidget : function (widgetTag) {
			...
			var css = "RfsSearch.css";
			if (widgetTag.extension.css)
				css = widgetTag.extension.css[0];
			var s = jQuery('head').find("link[href='" + Fusion.getFusionURL() + widgetTag.location + "RfsSearch/" + css + "']");
			if (s.length == 0) {
				Fusion.addWidgetStyleSheet(widgetTag.location + 'RfsSearch/' + css);
			}
		...
		onOpen : function (flyout) {
			if (!this.select1) {
				...
				this.input1 = jQuery("<input>", {
						'type' : 'text'
					}).keyup(this, this.search1);
				...
				if (ext.title2) {
					...
					this.input2 = jQuery("<input>", {
							'type' : 'text'
						}).keyup(this, this.search2);
				...
				if (ext.title3) {
					...
					this.input3 = jQuery("<input>", {
							'type' : 'text'
						}).keyup(this, this.search3);
